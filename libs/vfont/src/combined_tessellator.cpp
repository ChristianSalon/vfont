...
FT_Outline_LineToFunc CombinedTessellator::_lineToFunc = [](const FT_Vector* to, void* user) {
    ...
    uint32_t lastVertexIndex = CombinedTessellator::_getVertexIndex(CombinedTessellator::_currentGlyph.mesh.getVertices(), CombinedTessellator::_currentGlyphData.lastVertex);

    glm::vec2 newVertex = glm::vec2(static_cast<float>(to->x), static_cast<float>(to->y));
    int newVertexIndex = CombinedTessellator::_getVertexIndex(CombinedTessellator::_currentGlyph.mesh.getVertices(), newVertex);
    if (newVertexIndex == -1) {
        CombinedTessellator::_currentGlyph.mesh.addVertex(newVertex);
...
FT_Outline_ConicToFunc CombinedTessellator::_conicToFunc = [](const FT_Vector* control, const FT_Vector* to, void* user) {
    CombinedTessellator* pThis = reinterpret_cast<CombinedTessellator*>(user);

    uint32_t startPointIndex = CombinedTessellator::_getVertexIndex(CombinedTessellator::_currentGlyph.mesh.getVertices(), CombinedTessellator::_currentGlyphData.lastVertex);
    ...
    int controlPointIndex = CombinedTessellator::_getVertexIndex(CombinedTessellator::_currentGlyph.mesh.getVertices(), controlPoint);
    ...
    int endPointIndex = CombinedTessellator::_getVertexIndex(CombinedTessellator::_currentGlyph.mesh.getVertices(), endPoint);
    if (endPointIndex == -1) {
    ...
    if (CombinedTessellator::_isOnRightSide(vertices.at(curveSegment.start), vertices.at(curveSegment.end), vertices.at(curveSegment.control))) {
...
Glyph CombinedTessellator::composeGlyph(uint32_t codePoint, std::shared_ptr<vft::Font> font) {
    ...
    /*for (vft::Curve curve : curveSegments) {
        if (CombinedTessellator::_isOnRightSide(vertices.at(curve.start), vertices.at(curve.end), vertices.at(curve.control))) {
