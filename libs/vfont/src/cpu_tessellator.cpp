...
FT_Outline_LineToFunc CpuTessellator::_lineToFunc = [](const FT_Vector* to, void* user) {
    ...
    uint32_t lastVertexIndex = CpuTessellator::_getVertexIndex(CpuTessellator::_currentGlyph.mesh.getVertices(), CpuTessellator::_currentGlyphData.lastVertex);
...
FT_Outline_ConicToFunc CpuTessellator::_conicToFunc = [](const FT_Vector* control, const FT_Vector* to, void* user) {
    ...
    uint32_t startPointIndex = CpuTessellator::_getVertexIndex(CpuTessellator::_currentGlyph.mesh.getVertices(), CpuTessellator::_currentGlyphData.lastVertex);
    ...
    int controlPointIndex = CpuTessellator::_getVertexIndex(CpuTessellator::_currentGlyph.mesh.getVertices(), controlPoint);
    ...
    if (newVertices.size() == 2) {
        ...
        for (float t : newVertices) {
            ...
            glm::vec2 newVertex{ (1 - t) * (1 - t) * vertices.at(curveSegment.start) + 2 * (1 - t) * t * vertices.at(curveSegment.control) + (t * t) * vertices.at(curveSegment.end) };
            ...
            pThis->_edges.push_back(vft::Edge{ static_cast<uint32_t>(lastVertexIndex), static_cast<uint32_t>(newVertexIndex) });
...
Glyph CpuTessellator::composeGlyph(uint32_t codePoint, std::shared_ptr<vft::Font> font) {
    ...
    /*for (vft::Curve curve : curveSegments) {
        std::map<float, glm::vec2> newVertices = this->_subdivideQuadraticBezier(vertices, curve);
        if (newVertices.size() == 2) {
            lineSegments.push_back(vft::Edge{ curve.start, curve.end });
        }
        else {
            int lastVertexIndex = CpuTessellator::_getVertexIndex(vertices, vertices.at(curve.start));

            for (std::map<float, glm::vec2>::iterator it = std::next(newVertices.begin(), 1); it != newVertices.end(); it++) {
                glm::vec2 newVertex = it->second;
                int newVertexIndex = CpuTessellator::_getVertexIndex(vertices, newVertex);
                if (newVertexIndex < 0) {
                ...
                lineSegments.push_back(vft::Edge{ static_cast<uint32_t>(lastVertexIndex), static_cast<uint32_t>(newVertexIndex) });
                lastVertexIndex = newVertexIndex;
            }
        }
    }*/
 ...
 */
void CpuTessellator::_subdivide(const std::array<glm::vec2, 3> curve, float t, float delta, std::set<float>& newVertices) {
