    ...
    try {
        ...
        for (int i = 1; i < argc; i++) {
            if (strcmp(argv[i], "-h") == 0) {
                // Show help message
                std::cout << "./benchmark [-h] [-c <perspective/orthographic>] [-a <cdt/ts/wn/sdf>] [-f path_to_font] "
                             "[-t] [-m]"
                          << std::endl;
        ...
        BenchmarkScene scene{cameraType, tessellationAlgorithm, font, useMsaa, measureTime};
        scene.run();
