...
const std::string MainWindow::DEFAULT_WINDOW_TITLE = "vfont-demo";
...
MainWindow::MainWindow() {
    ...
    this->setLeftDragCallback([](float, float) -> void {});
    this->setRightDragCallback([](float, float) -> void {});
...
LRESULT CALLBACK MainWindow::_wndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        ...
        case WM_LBUTTONDOWN: {
            ...
            SetCapture(hwnd);
            pThis->_leftMouseButtonDown = true;
        ...
        case WM_LBUTTONUP: {
            ...
            ReleaseCapture();
            pThis->_leftMouseButtonDown = false;
        ...
        case WM_RBUTTONDOWN: {
            ...
            SetCapture(hwnd);
            pThis->_rightMouseButtonDown = true;
            GetCursorPos(&pThis->_lastMousePosition);
        ...
        case WM_RBUTTONUP: {
            ...
            ReleaseCapture();
            pThis->_rightMouseButtonDown = false;
        ...
        case WM_MOUSEMOVE: {
            ...
            if (pThis->_leftMouseButtonDown) {
                ...
                pThis->_leftDragCallback(dx, dy);
                pThis->_lastMousePosition = currentMousePos;
            } else if (pThis->_rightMouseButtonDown) {
                POINT currentMousePos;
                GetCursorPos(&currentMousePos);
                ...
                pThis->_rightDragCallback(dx, dy);
                pThis->_lastMousePosition = currentMousePos;
            }
...
void MainWindow::create() {
    ...
    this->_hwnd = CreateWindowEx(0,                    // Optional window styles
...
MainWindow::MainWindow() {
    ...
    this->setResizeCallback([](int, int) -> void {});
    this->setLeftDragCallback([](float, float, bool) -> void {});
    this->setScrollCallback([](float) -> void {});
    this->setKeypressCallback([](uint32_t) -> void {});
...
void MainWindow::create() {
    ...
    this->_window = XCreateWindow(this->_display, RootWindow(this->_display, this->_screen), 0, 0, DEFAULT_WIDTH,
                                  DEFAULT_HEIGHT, 1, CopyFromParent, InputOutput, CopyFromParent, 0, nullptr);
    ...
    this->_inputContext = XCreateIC(this->_inputMethod, XNInputStyle, XIMPreeditNothing | XIMStatusNothing,
                                    XNClientWindow, this->_window, nullptr);
...
void MainWindow::pollEvents() {
    ...
    switch (ev.type) {
        ...
        case KeyPress: {
            ...
            Status status = 0;
            nOfCodeUnits = Xutf8LookupString(this->_inputContext, (XKeyPressedEvent *)&ev, buffer, maxBufferBytes,
                                             nullptr, &status);
...
struct wl_registry_listener MainWindow::_registryListener = {
    .global = [](void *data, struct wl_registry *registry, uint32_t name, const char *interface, uint32_t version) {
        ...
        if (strcmp(interface, wl_compositor_interface.name) == 0) {
            ...
            pThis->_decorationManager = static_cast<zxdg_decoration_manager_v1 *>(
...
struct xdg_wm_base_listener MainWindow::_xdgWmBaseListener = {
    .ping = [](void *data, struct xdg_wm_base *xdgWmBase, uint32_t serial) { xdg_wm_base_pong(xdgWmBase, serial); }};
...
struct xdg_toplevel_listener MainWindow::_xdgToplevelListener = {
    .configure =
        [](void *data, struct xdg_toplevel *xdgToplevel, int32_t width, int32_t height, struct wl_array *states) {
            MainWindow *pThis = reinterpret_cast<MainWindow *>(data);
    ...
    .close =
        [](void *data, struct xdg_toplevel *xdgToplevel) {
            MainWindow *pThis = reinterpret_cast<MainWindow *>(data);
...
struct wl_keyboard_listener MainWindow::_wlKeyboardListener = {
    ...
    .keymap =
        [](void *data, struct wl_keyboard *wl_keyboard, uint32_t format, int32_t fd, uint32_t size) {
            ...
            pThis->_xkbKeymap = xkb_keymap_new_from_string(pThis->_xkbContext, keymap_string, XKB_KEYMAP_FORMAT_TEXT_V1,
...
struct wl_pointer_listener MainWindow::_wlPointerListener = {
    ...
    .motion =
        [](void *data, wl_pointer *pointer, uint32_t time, wl_fixed_t x, wl_fixed_t y) {
            ...
            // Check if left button of mouse is pressed
            if (pThis->pThis->_leftMouseButtonDown = true;) {
                int deltaX = newX - lastMouseX;
                int deltaY = newY - lastMouseY;

                pThis->_leftDragCallback(deltaX, deltaY);
            }  // Check if right button of mouse is pressed
            else if (pThis->pThis->_leftMouseButtonDown = true;) {
                int deltaX = newX - lastMouseX;
                int deltaY = newY - lastMouseY;

                pThis->_rightDragCallback(deltaX, deltaY);
            }
    ...
    .button =
        [](void *data, wl_pointer *pointer, uint32_t serial, uint32_t time, uint32_t button, uint32_t state) {
            ...
            if (button == 0x0110 && state == WL_POINTER_BUTTON_STATE_PRESSED) {
                ...
                pThis->_leftMouseButtonDown = false;
            } else if (button == 0x0111 && state == WL_POINTER_BUTTON_STATE_PRESSED) {
                pThis->_rightMouseButtonDown = true;
            } else if (button == 0x0111 && state == WL_POINTER_BUTTON_STATE_RELEASED) {
                pThis->_rightMouseButtonDown = false;
            }
        },
    .axis =
...
void MainWindow::create() {
    ...
    xkb_compose_table *composeTable =
        xkb_compose_table_new_from_locale(this->_xkbContext, locale, XKB_COMPOSE_COMPILE_NO_FLAGS);
    ...
    if (this->_decorationManager) {
        ...
        this->_toplevelDecoration =
            zxdg_decoration_manager_v1_get_toplevel_decoration(this->_decorationManager, this->_xdgToplevel);
 ...
 * @param resizeCallback Callback function
 */
void MainWindow::setLeftDragCallback(std::function<void(float, float)> dragCallback) {
    this->_leftDragCallback = dragCallback;
}
 ...
 * @param resizeCallback Callback function
 */
void MainWindow::setRightDragCallback(std::function<void(float, float)> dragCallback) {
    this->_rightDragCallback = dragCallback;
}
